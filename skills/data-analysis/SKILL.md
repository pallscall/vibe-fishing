...
- **File location**: Path(s) to uploaded Excel/CSV files under `/tmp/user-data/uploads/`
...
python /mnt/skills/public/data-analysis/scripts/analyze.py \
  --files /tmp/user-data/uploads/data.xlsx \
  --action inspect
```
...
python /mnt/skills/public/data-analysis/scripts/analyze.py \
  ...
  --output-file /tmp/user-data/outputs/filtered-results.csv
```
...
```bash
python /mnt/skills/public/data-analysis/scripts/analyze.py \
  --files /tmp/user-data/uploads/sales_2024.xlsx \
  --action inspect
```
...
python /mnt/skills/public/data-analysis/scripts/analyze.py \
  ...
  --output-file /tmp/user-data/outputs/monthly-trends.csv
```
...
```bash
python /mnt/skills/public/data-analysis/scripts/analyze.py \
  --files /tmp/user-data/uploads/orders.csv /tmp/user-data/uploads/customers.xlsx \
  --action query \
...
- On first load, files are parsed and stored in a persistent DuckDB database under `/tmp/user-data/workspace/.data-analysis-cache/`
